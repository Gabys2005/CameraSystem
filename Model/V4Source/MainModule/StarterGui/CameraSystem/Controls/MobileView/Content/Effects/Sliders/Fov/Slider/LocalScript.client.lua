...
replicated.Server.Fov.Changed:Connect(function(val)
	script.Parent.Parent.TextLabel.Text = "FOV: " .. math.round(val)
	script.Parent.Frame.Size = UDim2.new(0,((val-1)/119)*script.Parent.AbsoluteSize.X,1,0)
end)

local val = replicated.Server.Fov.Value
script.Parent.Parent.TextLabel.Text = "FOV: " .. math.round(val)
script.Parent.Frame.Size = UDim2.new(0,((val-1)/119)*script.Parent.AbsoluteSize.X,1,0)
