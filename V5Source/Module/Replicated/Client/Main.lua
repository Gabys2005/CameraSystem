...
return function(gui) -- TODO: use this param
	-- Setup springs
	...
	if table.find(barsOffset.Players, localPlayer.Name) then
		gui.Bars.Position = UDim2.fromOffset(0, barsOffset.Offset)
		gui.Bars.Size = UDim2.new(1, 0, 1, -barsOffset.Offset)
	end
	...
	replicated.Events.SmoothFocus.OnClientEvent:Connect(function(bool)
		if bool then
			data:get("Local.Springs.Focus").Position =
				utils:CFrameToRotation(CFrame.lookAt(data:get("Shared.CameraData.Position"), utils:getFocusPosition()))
		end
